...
PowerManager::PowerManager(QObject *parent)
    : QObject(parent)
    , m_iface("com.long.PowerManager",
              "/CPUManagement", "com.long.CPUManagement",
    ...
    , m_mode(-1)
{
    ...
    QSettings settings(QSettings::UserScope, "longos", "power");
...
void PowerManager::setIdleTime(int idleTime)
{
    if (m_idleTime != idleTime) {
        ...
        QDBusInterface iface("com.long.PowerManager",
                             "/PowerManager", "com.long.PowerManager",
                             QDBusConnection::sessionBus());
        if (iface.isValid()) {
