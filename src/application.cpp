...
const QString ModuleDirectory = "/usr/lib/long-settings/modules";
    ...
    : QApplication(argc, argv)
{
    ...
    parser.setApplicationDescription(QStringLiteral("Long Settings"));
    ...
    if (!QDBusConnection::sessionBus().registerService("com.long.SettingsUI")) {
        QDBusInterface iface("com.long.SettingsUI", "/SettingsUI", "com.long.SettingsUI", QDBusConnection::sessionBus());
        if (iface.isValid())
    ...
    // Translations
    QLocale locale;
    QString qmFilePath = QString("%1/%2.qm").arg("/usr/share/long-settings/translations/").arg(locale.name());
