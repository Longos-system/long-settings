...
bool About::isLongOS()
{
    if (!QFile::exists("/etc/longos"))
        return false;

    QSettings settings("/etc/longos", QSettings::IniFormat);
    return settings.value("LongOS", false).toBool();
}
...
{
    QProcess::startDetached("long-terminal", QStringList());
}
